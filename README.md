...
Seeklume 是面向高中学生的英语学习网站。新方向以真实的文本和音频为载体，通过精听、阅读、Shadow Reading、录音回听和专项练习，帮助学生完成“理解输入 → 模仿表达 → 回听改进”的学习循环。

线上入口：

- GitHub Pages 发布目录：`docs/`
- 自定义域名：`https://seeklume.work/`
- 当前主模块：`docs/read-along/`（Shadow Reading）

## 当前范围（2026-09 起）

当前首要服务用户是自有班级的高中学生。第一阶段先把 Shadow Reading 做稳定：逐句音频、跟读间隔、录音回听和本机完成记录。语法填空保留为次级的专项练习，不再是整个产品的中心。

教师管理功能、班级工作台和礼堂排座暂停发展，从学生界面下线并保留代码档案。学生录音首版只在当前设备中回听，不上传。新内容优先使用原创、获得授权或明确开放授权的文本和音频；暂不建设视频托管功能。
