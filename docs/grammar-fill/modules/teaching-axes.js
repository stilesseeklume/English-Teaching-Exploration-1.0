...
//
// L1 ESM 试点：本模块改为浏览器原生 ES module（export）。底部保留
// window.GrammarTeachingAxes 全局桥，兼容现有调用方（如 index.html 的 getNonpAxis 包装）。
// 等调用方都改用 import 后，可删除底部全局桥。

/* eslint-disable */
export const NONP_FUNCTION_LABELS = {
...
// 迁移期全局桥：维持现有 window.GrammarTeachingAxes 调用方（index.html:606）。
// 迁完调用方改 import 后，此块可删。
