...
> **2026-06-10 范围调整（安全先行）：** ~~套卷 Word→AI 拖拽推迟~~。已完成：Task1 引擎（bbdd135）、成绩拖拽区 + 1.5 智能匹配（58e8332）。
>
> **2026-06-10 重新纳入（负责人明确要全）：** 要的是和「导入试题(备课)」一样的体验——**两个虚线拖拽框：① 套卷 Word ② 成绩 .xls**，两个都拖好后再 AI 分析 + 匹配 → 画像进班。样式照搬备课的 dashed 框「把 Word 卷子拖进来，或点击上传」。
>
> **套卷 Word→AI 的洁净复用法（不另写 headless）：** 给 `word-import.js` 加一个 `'exam-profile'` 目标——复用现成的 `handleDroppedFile → processDocxFile → parseWithDeepSeek` 全流程（含 AI 进度浮层），只在 `parseWithDeepSeek` 末尾把 `passagesArr` 交给回调（而不是 `openUnifiedImportPanel` 那个备课/错题面板）。blank 形状 `{no, answer, category, fine_category?, analysis}`。
> 引擎加纯函数 `extractGrammarBlanks(passagesArr)` 拍平所有 blank→`[{no,category,answer,fine_category}]` 按题号升序去重。app `parseExamWord(file)` 包成 Promise。AI 端仍只能线上验。
